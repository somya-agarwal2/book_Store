...
cloudinary.config({
  cloud_name:'dvyf4wvd2',
  api_key:'948948616598599',
  api_secret:'S1KJIsoPQjEY00YCMDuWr6DA8js'
});
...
router.get('/',(req,res,next)=>{
  Product.find()
  .select('_id title thumbnail author price')
  .then(result=>{
    res.status(200).json({
      product:result
    })
...
router.get('/search',(req,res)=>{
  ...
  const products=Product.find({title:{$regex:q,$options:'i'}})
  .select('_id title thumbnail author price')
  .then(result=>{
    res.status(200).json({
      product:result
